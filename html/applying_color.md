...
tags:
  ...
  - Styling HTML
  - color
permalink: html/applying_color
mdn: 'https://developer.mozilla.org/en-US/docs/Web/HTML/Applying_color'
---
{{ HTMLRef }}

The use of color is a fundamental form of human expression. Children experiment with color before they even have the manual dexterity to draw. Maybe that's why color is one of the first things people often want to experiment with when learning to develop websites. With [CSS](/css), there are lots of ways to add color to your [HTML](/html) [elements](/html/element) to create just the look you want. This article is a primer introducing each of the ways CSS color can be used in HTML.
...
{{ cssxref("caret-color") }}
: The color to use when drawing the [caret](/glossary/caret/) (sometimes referred to as the text input cursor) within the element. This is only useful in elements that are editable, such as [`input`](/html/element/input/) and [`textarea`](/html/element/textarea/) or elements whose HTML {{ htmlattrxref("contenteditable") }} attribute is set.
...
Any element can have a [border](/en-US/docs/Learn/CSS/Styling_boxes/Borders) drawn around it. A basic element border is a line drawn around the edges of the element's content. See {{ SectionOnPage("/en-US/docs/Learn/CSS/Introduction_to_CSS/Box_model", "Box properties") }} to learn about the relationship between elements and their borders, and the article [Styling borders using CSS](/en-US/docs/Learn/CSS/Styling_boxes/Borders) to learn more about applying styles to borders.

You can use the {{ cssxref("border") }} shorthand property, which lets you configure everything about the border in one shot (including non-color features of borders, such as its [width](/css/border-width), [style](/css/border-style) (solid, dashed, etc.), and so forth.
...
The HTML [Canvas API](/api/canvas_api)
: Lets you draw 2D bitmapped graphics in a [`canvas`](/html/element/canvas/) element. See our [Canvas tutorial](/api/canvas_api/tutorial) to learn more.

[SVG](/svg) (Scalable Vector Graphics)
: Lets you draw images using commands that draw specific shapes, patterns, and lines to produce an image. SVG commands are formatted as XML, and can be embedded directly into a web page or can be placed in the page using the [`img`](/html/element/img/) element, just like any other type of image.

[WebGL](/api/webgl_api)
: The Web Graphics Library is an OpenGL ES-based API for drawing high-performance 2D and 3D graphics on the Web. See our [WebGL tutorial](/api/webgl_api/tutorial) to find out more.
...
Designers and artists often prefer to work using the [`HSL and HSV`](https://en.wikipedia.org/wiki/HSL_and_HSV) (Hue/Saturation/Luminosity) color method. On the web, HSL colors are represented using HSL functional notation. The `hsl()` CSS function is very similar to the `rgb()` function in usage otherwise.
...
This is pretty simple, using a [`div`](/html/element/div/) as a wrapper around the contents, which consists of two more `<div>`s, each styled differently with a single paragraph ([`p`](/html/element/p/)) in each box.
...
The `.wrapper` class is used to assign styles to the [`div`](/html/element/div/) that encloses all of our other content. This establishes the size of the container using {{ cssxref("width") }} and {{ cssxref("height") }} as well as its {{ cssxref("margin") }} and {{ cssxref("padding") }}.
...
In brief, `.box` establishes the size of each box, as well as the configuration of the font used within. We also take advantage of [CSS Flexbox](/css/css_flexible_box_layout) to easily center the contents of each box. We enable `flex` mode using {{ cssxref("display", "display: flex") }}, and set both {{ cssxref("justify-content") }} and {{ cssxref("align-items") }} to `center`. Then we can create a class for each of the two boxes that defines the properties that differ between the two.
...
There are many situations in which your website may need to let the user select a color. Perhaps you have a customizable user interface, or you're implementing a drawing app. Maybe you have editable text and need to let the user choose the text color. Or perhaps your app lets the user assign colors to folders or items. Although historically it's been necessary to implement your own [`color picker`](https://en.wikipedia.org/wiki/color_picker), HTML now provides support for browsers to provide one for your use through the [`input`](/html/element/input/) element, by using `"color"` as the value of its {{ htmlattrxref("type", "input") }} attribute.
...
The HTML here creates a box that contains a color picker control (with a label created using the [`label`](/html/element/label/) element) and an empty paragraph element ([`p`](/html/element/p/)) into which we'll output some text from our JavaScript code.
...
The script here handles the task of updating the starting color of the border to match the color picker's value. Then two event handlers are added to deal with input from the `[<input type="color">](/html/element/input/color)` element.
...
-   [MDN's color picker tool](/css/css_colors/color_picker_tool)
...
There are several ways color can be an [accessibility](/glossary/accessibility/) problem. Improper or careless use of color can result in a website or app that a percentage of your target audience may not be able to use adequately, resulting in lost traffic, lost business, and possibly even a public relations problem. So it's important to consider your use of color carefully.
...
-   [Graphics on the web](/guide/graphics)
-   [MDN's color picker tool](/css/css_colors/color_picker_tool)